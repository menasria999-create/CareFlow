...
const DoctorProfile = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const requiredFields = ['full_name', 'email', 'phone', 'specialization', 'license_number', 'years_experience', 'clinic_address', 'bio'];
        let missing = false;
        ...
        try {
            ...
            if (response.data.success) {
                ...
                throw new Error(response.data.message || 'فشل التحديث');
            }
        } catch (error) {
            setMessage({ text: error.response?.data?.error || error.message || '❌ حدث خطأ في التحديث', type: 'error' });
        } finally {
            setSaving(false);
        }
    };
    ...
    const handlePasswordChange = async (e) => {
        ...
        try {
            const response = await api.put('/doctors/change-password', {
        ...
        <div style={styles.container}>
            <div style={styles.sidebar}>
                ...
                <div style={styles.userInfo}>
                    ...
                    <div style={styles.userName}>{user?.full_name}</div>
                    <div style={styles.userRole}>طبيب</div>
                </div>
                <nav style={styles.nav}>
                    {menuItems.map((item) => (
                        <button key={item.path} onClick={() => navigate(item.path)} style={styles.navItem}>
            ...
            <div style={styles.mainContent}>
                <div style={styles.header}>
                    <h1>ملفي الشخصي</h1>
                    <p>جميع الحقول التالية إجبارية (ما عدا حقل التخصص للقراءة فقط)</p>
                </div>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    {/* صف 2: رقم الهاتف + التخصص (للقراءة فقط) */}
                    <div style={styles.formRow}>
                        <div style={styles.halfInput}>
                        ...
                        <div style={styles.halfInput}>
                            <label style={styles.label}>الاختصاص (التخصص)</label>
                            <input type="text" name="specialization" value={profile.specialization || ''} readOnly disabled style={{...styles.input, ...styles.readonlyInput}} />
                            <span style={styles.readonlyNote}>لا يمكن تغيير التخصص</span>
                        </div>
                    </div>

                    {/* صف 3: رقم الرخصة + سنوات الخبرة */}
                    <div style={styles.formRow}>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>رقم الرخصة المهنية <span style={styles.requiredStar}>*</span></label>
                            <input type="text" name="license_number" value={profile.license_number || ''} onChange={handleChange} required style={styles.input} />
                        </div>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>سنوات الخبرة <span style={styles.requiredStar}>*</span></label>
                            <input type="number" name="years_experience" value={profile.years_experience || ''} onChange={handleChange} required min="0" style={styles.input} />
                        </div>
                    </div>

                    {/* صف 4: عنوان العيادة + نبذة عني */}
                    <div style={styles.formRow}>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>عنوان العيادة <span style={styles.requiredStar}>*</span></label>
                            <input type="text" name="clinic_address" value={profile.clinic_address || ''} onChange={handleChange} required style={styles.input} />
                        </div>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>نبذة عني <span style={styles.requiredStar}>*</span></label>
                            <textarea name="bio" value={profile.bio || ''} onChange={handleChange} rows="3" required placeholder="مثال: خبرة في مجال..." style={styles.textarea} />
                        </div>
                    </div>
