...
const Profile = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const requiredFields = ['full_name', 'email', 'phone', 'date_of_birth', 'blood_type', 'allergies', 'medical_history', 'emergency_contact', 'emergency_phone'];
        let missing = false;
            ...
            <div style={styles.mainContent}>
                ...
                <form onSubmit={handleSubmit} style={styles.form}>
                    ...
                    <div style={styles.formRow}>
                        ...
                        <div style={styles.halfInput}>
                            <label style={styles.label}>تاريخ الميلاد <span style={styles.requiredStar}>*</span></label>
                            <input type="date" name="date_of_birth" value={profile.date_of_birth ? formatDate(profile.date_of_birth) : ''} onChange={handleChange} required style={styles.input} />
                        </div>
                    </div>

                    {/* صف 3: فصيلة الدم + الحساسية */}
                    <div style={styles.formRow}>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>فصيلة الدم <span style={styles.requiredStar}>*</span></label>
                            <select name="blood_type" value={profile.blood_type || ''} onChange={handleChange} required style={styles.select}>
                                <option value="">-- اختر فصيلة الدم --</option>
                                {bloodTypes.map(type => <option key={type} value={type}>{type}</option>)}
                            </select>
                        </div>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>حساسية من أدوية <span style={styles.requiredStar}>*</span></label>
                            <textarea name="allergies" value={profile.allergies || ''} onChange={handleChange} rows="3" required placeholder="مثال: البنسلين، الأسبرين..." style={styles.textarea} />
                        </div>
                    </div>

                    {/* صف 4: التاريخ الطبي + جهة اتصال للطوارئ */}
                    <div style={styles.formRow}>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>التاريخ الطبي <span style={styles.requiredStar}>*</span></label>
                            <textarea name="medical_history" value={profile.medical_history || ''} onChange={handleChange} rows="3" required placeholder="مثال: السكري، ضغط الدم..." style={styles.textarea} />
                        </div>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>جهة اتصال للطوارئ <span style={styles.requiredStar}>*</span></label>
                            <input type="text" name="emergency_contact" value={profile.emergency_contact || ''} onChange={handleChange} required placeholder="اسم الشخص" style={styles.input} />
                        </div>
                    </div>

                    {/* صف 5: رقم هاتف الطوارئ */}
                    <div style={styles.formRow}>
                        <div style={styles.halfInput}>
                            <label style={styles.label}>رقم هاتف الطوارئ <span style={styles.requiredStar}>*</span></label>
                            <input type="tel" name="emergency_phone" value={profile.emergency_phone || ''} onChange={handleChange} required placeholder="رقم الهاتف" style={styles.input} />
                        </div>
                        <div style={styles.halfInput}>
